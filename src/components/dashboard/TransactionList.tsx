...
import type { TransactionData, AccountWithBalance } from "@/app/(dashboard)/dashboard/page";
...
export function TransactionList({ transactions, accounts, selectedAccountId, onDelete }: TransactionListProps) {
    ...
    const filteredTransactions = selectedAccountId
        ? transactions.filter((t) => t.accountId === selectedAccountId || t.transferToAccountId === selectedAccountId)
        : transactions;
    ...
    const getTransferLabel = (transaction: TransactionData) => {
        if (transaction.type !== "TRANSFER") return null;

        const isIncomingTransfer = transaction.transferToAccountId === selectedAccountId;

        if (isIncomingTransfer) {
            // Money coming in - show the "from" account
            const fromAccount = accounts.find((a) => a.id === transaction.accountId);
            return `from ${fromAccount?.name || "Unknown"}`;
        } else {
            // Money going out - show the "to" account
    ...
    const getTransactionDisplay = (transaction: TransactionData) => {
        ...
        if (isTransfer) {
            return {
                ...
                badgeClass: isIncomingTransfer ? "bg-sage-100 text-sage-700" : "bg-blue-100 text-blue-700",
    ...
    return (
        <Card>
            <h3 className="text-lg font-semibold text-sage-900 mb-4">Recent Transactions</h3>

            {currentTransactions.length === 0 ? (
                <div className="text-center py-12">
                    <p className="text-sage-500">No transactions yet</p>
                    <p className="text-sage-400 text-sm mt-1">Add your first transaction to get started</p>
                </div>
            ) : (
                                        ...
                                        <p className="text-sm text-sage-500">
                                            ...
                                            {isTransfer && (
                                                <span className="ml-2 text-blue-500">
                                                    • Transfer {getTransferLabel(transaction)}
                                                </span>
                                            )}
                                        </p>
                                    </div>
